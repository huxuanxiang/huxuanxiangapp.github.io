<template>
  <div id="PerSon">
      <div class="Person-top-box">
          <!-- user用户img界面部分 -->
         <div class="top">
             <!-- 头像 -->
             <div class="top_proto">
                 <div class="proto_left">
                     <div><img src="" alt=""></div>
                     <div>158****5021</div>
                     <div><img src="../../assets/images/修改@2x.png" alt=""></div>
                 </div>
                 <div class="proto_right">去约课</div>
             </div>
             <!-- 特色课程 -->
             <div class="top_study">
                 <div class="first">
                     <div>0</div>
                     <div>我的特色课</div>
                     <div>已购特色课程的学习</div>
                 </div>
                 <div class="first">
                     <div>0</div>
                     <div>一对一辅导</div>
                     <div>我的一对一老师辅导</div>
                 </div>
                 <div class="first">
                     <div>0.00</div>
                     <div>剩余学习币</div>
                     <div>查看剩余学习币</div>
                 </div>
             </div>
         </div>

      </div>
<!-- 邀请注册 -->
    <div class="among">
      <img
        src="data:image/png;base64,"
        alt
      />
      <div class="inner">
        <div>邀请好友注册APP，享多重好礼</div>
        <div>限时特惠，多邀多得</div>
      </div>
      <van-icon name="arrow" size="20" color="white" />
    </div>

    <!-- 课程相关 -->
    <div class="foot">
      <div class="of">
        <div>课程相关</div>
        <div class="sof" @click="toConcern">
          <img
            src="data:image/png;base64,"
            alt
          />
          <div>关注的老师</div>
        </div>
        <div class="sof" @click="toCollect">
          <img
            src="data:image/png;base64,"
            alt
          />
          <div>我的收藏</div>
        </div>
      </div>
      <div class="fh"></div>
      <div class="of">
        <div>订单相关</div>
        <div class="sof">
          <img
            style="width:0.18rem"
            src="data:image/png;base64,"
            alt
          />
          <div>课程订单</div>
        </div>
        <div class="sof">
          <img
            style="width:0.18rem"
            src="data:image/png;base64,"
            alt
          />
          <div>会员订单</div>
        </div>
        <div class="sof">
          <img
            style="width:0.18rem"
            src="data:image/png;base64,"
            alt
          />
          <div>约课订单</div>
        </div>
      </div>
      <div class="fh"></div>
      <div class="of">
        <div>我的账户</div>
        <div class="sof">
          <img
            style="width:0.2rem"
            src="data:image/png;base64,"
            alt
          />
          <div>优惠券</div>
        </div>
        <div class="sof">
          <img
            src="data:image/png;base64,"
            alt
          />
          <div>学习卡</div>
        </div>
        <div class="sof">
          <img
            style="width:0.22rem"
            src="data:image/png;base64,"
            alt
          />
          <div>会员</div>
        </div>
      </div>
      <div class="fh"></div>
      <div class="of">
        <div>自助服务</div>
        <div class="sof">
          <img
            src="data:image/png;base64,"
            alt
          />
          <div>我的消息</div>
        </div>
        <div class="sof">
          <img
            src="data:image/png;base64,"
            alt
          />
          <div>意见反馈</div>
        </div>
        <div class="sof">
          <img
            src="data:image/png;base64,"
            alt
          />
          <div>在线客服</div>
        </div>
        <div class="sof">
          <img
            src="data:image/png;base64,"
            alt
          />
          <div>设置</div>
        </div>
      </div>
    </div>
    <!-- 灰色区域 -->
    <div class="hs"></div>


  </div>
</template>

<script>
export default {
name:"PerSon",
data() {
    return {
        
    }

},
methods: {
    // 跳转关注页面
    toConcern(){
        this.$router.push("/concern")
    },
        // 跳转收藏页面
    toCollect(){
        this.$router.push("/collect")
    }
},
//生命周期 - 创建完成（访问当前this实例）
created() {

},
//生命周期 - 挂载完成（访问DOM元素）
mounted() {

}
}
</script>
<style lang="scss">
/* @import url(); 引入css类 */
#PerSon{
    overflow: hidden;
    padding-bottom: 2rem;
    padding-bottom: 20vw;
}

.Person-top-box{
    height: 5.8rem;
    height: 58vw;
    background: url('../../assets/img/user_bg.png') no-repeat no-repeat;
    background-size: 100% 1.3rem;
    background-color: #fff;
    position: relative;
    padding: 0 .4rem;
    padding: 0 4vw;
    background-color: lightgrey;
    position:relative;
}
/* 头像部分 */
.top
{
    width: 94%;
    height: 1.6rem;
    background-color: #fff;
    position: absolute;
    bottom:0.15rem;
    border-radius: 0.06rem;
}
.top_proto
{
    width: 100%;
    height: 0.7rem;
    display:flex;
    justify-content: space-between;
    align-items: center;
    margin-top:0.1rem;
}
.proto_left
{
    width: 80%;
    height: 0.7rem;
    display:flex;
    justify-content:flex-start;
    align-items: center;
}
.proto_left>div:nth-child(1){
    width: 23%;
    height: 0.6rem;
    background-color: pink;
    margin-left:5%;
    border-radius: 50%;
}
.proto_left>div:nth-child(2){
    width: 30%;
    margin-left:3%;
}
.proto_left>div:nth-child(3){
    width: 20%;
    margin-left:3%;
}
.proto_left>div:nth-child(3)>img{
   width: 28%;
   height: 0.16rem;
}

.proto_right
{
    width: 16%;
    height: 0.25rem;
    color:#fff;
    background-color: tomato;
    display:flex;
    justify-content: center;
    align-items: center;
    border-top-left-radius: 0.15rem;
    border-bottom-left-radius: 0.15rem;

}
/* 特色课 */
.top_study{
    width: 100%;
    height: 0.7rem;
    display:flex;
    justify-content: space-around;
    align-items: center;
}
.top_study>div
{
    width: 33%;
    text-align: center;
    border-right:1px solid lightgrey;
}
.top_study>div:last-child
{
    border-right:none
}
.first>div:nth-child(1)
{
    color: red;
    font-size:0.22rem;
}
.first>div:nth-child(3)
{
    color: lightgrey;
    font-size:0.11rem;
}
/* 邀请注册 */
.among {
  width: 92%;
  height: 0.45rem;
  margin-left: 4%;
  background: linear-gradient(to right, #eb6406, #f1904c);
  border-radius:0.5rem;
  position: relative;
  img {
    width: 0.2rem;
    height: 0.2rem;
    margin-left: 0.2rem;
    float: left;
    margin-top: 0.12rem;
  }
  .van-icon {
    position: absolute;
    margin-top: 0.12rem;
    right: 0.15rem;
  }
  .inner {
    width: 1.8rem;
    float: left;
    color: white;
    margin-left:0.15rem;
    margin-top:0.06rem;
    height: 0.5rem;

    div:nth-child(1) {
      font-size: 0.12rem;
    }
    div:nth-child(2) {
      font-size:0.115rem;
      margin-top: 4px;
    }
  }
}

// 
.foot {
  .fh {
    width: 90%;
    height: 1px;
    background-color: rgb(243, 243, 243);
    margin-left: 5%;
  }
  margin-top: 20px;
  .of {
    width: 92%;
    margin-top: 0.1rem;
    margin-left: 4%;
    height: 0.86rem;
    div {
      margin: 0;
    }
    div {
      margin-left: 0.01rem;
    }
    img {
      width: 0.20rem;
      height: 0.20rem;
      margin-left: 30%;
    }
    .sof {
      margin-top: 0.08rem;
      width: 25%;
      float: left;
      div:nth-child(2) {
        font-size: 0.12rem;
        text-align: center;
        color: rgb(148, 148, 146);
      }
    }
  }
}

</style>